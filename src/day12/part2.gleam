...
pub fn main() {
  ...
  |> list.map(fn(region) {
    let #(_, _, squares) = region
    let corners = squares |> list.map(count_corners(_, squares)) |> int.sum
    corners * list.length(squares)
  })
  |> int.sum
  |> io.debug
}

fn count_corners(c: utils.Coordinate, others: List(utils.Coordinate)) -> Int {
  [[Up, Right], [Up, Left], [Down, Right], [Down, Left]]
  |> list.map(fn(edges) {
    let assert [first, second] = edges
    case
      list.find(others, fn(x) { x == next_pos(c, first) }),
      list.find(others, fn(x) { x == next_pos(c, second) })
    {
      // Outer corner
      Error(_), Error(_) -> 1
      Ok(_), Ok(_) ->
        case
          list.find(others, fn(x) {
            x == c |> next_pos(first) |> next_pos(second)
          })
        {
          // Inner corner
          Error(_) -> 1
